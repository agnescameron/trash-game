...
module.exports = {
	...
	onboard: async (STATE) => {
		...
		await keypress();
		log(chalk.bold(characters.boss, `here's how the menu starts: as you get more buildings, more items will be added`));
		await keypress();
...
`));
		...
		log(chalk.bold(`to manage the off-site relationship, press 'o'
			...
			press 'e' to exit`));
		await keypress();
		log(chalk.bold(characters.boss, `as you acquire items, the percentage of recycling will increase`));
		await keypress();
		...
		await keypress();	
		log(chalk.bold(characters.boss, `and to run outreach campaigns, press 'e'`));
		await keypress();		
	},
